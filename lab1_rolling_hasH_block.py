...
def build_hash_table(S, max_len, min_len=100, block_size=1000):
    rh = RollingHash()
    ...
    for start in range(0, n, block_size):
        ...
        for k in range(min_len, max_len+1):
            if k > len(block): break

            # 预计算初始哈希
            current_hash = rh.get_hash(block[:k])
            hash_table.setdefault(current_hash, []).append((start, block[:k]))
            
            # 滑动窗口更新哈希
            for i in range(1, len(block) - k + 1):
                prev_char = block[i-1]
                new_char = block[i+k-1]
                current_hash = rh.update_hash(current_hash, prev_char, new_char, k)
                sub = block[i:i+k]
                hash_table.setdefault(current_hash, []).append((start + i, sub))
                
                # 存储反向互补
                rc_sub = reverse_complement(sub)
                rc_hash = rh.get_hash(rc_sub)
                hash_table.setdefault(rc_hash, []).append((start + i, rc_sub))
    
    return hash_table

def find_repeats_optimized(S_reference, S_query, max_len=float('inf'), min_len=40, block_size=1000):
    n, m = len(S_reference), len(S_query)
    hash_table = build_hash_table(S_reference, max_len, min_len, block_size)
